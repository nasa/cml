...
# Test 2 exhaust-speed is the missing variable in this equation.

exec(open("Modified_data/env_setup.py").read())
exec(open("Log_data/log_vent_params.py").read())
log_full_params()

# For code-coverage, going to set exhaust-speed to an invalid value, which results
# in zeroing the value and unsetting exhaust-speed.
