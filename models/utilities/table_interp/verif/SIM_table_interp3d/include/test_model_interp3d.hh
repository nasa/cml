/******************************************************************************
PURPOSE:
    (Test interpolation functions on three dimensional table)
...
#ifndef TEST_MODEL_INTERP3D_HH
#define TEST_MODEL_INTERP3D_HH
...
{
  return x*(1.0+std::cos(x)) + 1.5*y*(2.0+std::sin(y)) + 0.5*z*(1.0+std::sin(z));
}

template<int K, int N, int M>
class TestModelInterp3D: public TestModelInterp<K, N, M>
{
public:
  typedef TestModelInterp<K, N, M> Base;
  ...
  TestModelInterp3D(const double base_in=-2.0,  const double step_in=0.5) : 
    base(base_in), step(step_in), low_index_x(0), frac_x(0.0),
    low_index_y(0), frac_y(0.0), low_index_z(0), frac_z(0.0)
  {
    static_assert(0<N&&0<M&&0<K , "Dimension N, M and K should be great than zero.");
  }
 
  double update_impl(const DblTableType &var_list) const
  {
    static const double (&bpt_tbl_x)[N] = std::get<1>(Base::bpt_table_list);
    static const double (&bpt_tbl_y)[M] = std::get<2>(Base::bpt_table_list);
    static const double (&bpt_tbl_z)[K] = std::get<0>(Base::bpt_table_list);
    ...
    return table3(&(Base::val_table[0][0][0]), frac_x, low_index_x, N,
                  frac_y, low_index_y, M, frac_z, low_index_z);
  }
   
  void set_data(const bool asc, BptTableList &bpt_tbl_list, ValTableType &val_tbl) const 
  {
    static double (&bpt_tbl_x)[N] = std::get<1>(bpt_tbl_list);
    static double (&bpt_tbl_y)[M] = std::get<2>(bpt_tbl_list);
    static double (&bpt_tbl_z)[K] = std::get<0>(bpt_tbl_list);
    ...
    for (int k=0; k<K; ++k)
      for (int i=0; i<N; ++i)
        for (int j=0; j<M; ++j)
          val_tbl[k][i][j] = cal_val(bpt_tbl_x[i], bpt_tbl_y[j], bpt_tbl_z[k]);
  }
...
private:
  TestModelInterp3D(const TestModelInterp3D&);
  TestModelInterp3D& operator=(const TestModelInterp3D&); 
