...
class ConvertString {
 ...
 private:
  template<typename T> static T convert_numeric(const std::string& str);
};
...
template<typename T> T ConvertString::convert(const std::string& str) {
  if (str.empty()) {
    CMLMessage::error(__FILE__, __LINE__, "Bad Pointer\n",
      "An empty string was passed into ConvertString::convert.\n");
    return 0;
  }
...
template <> inline
float ConvertString::convert_numeric<float>(const std::string& str) {
  return strtof(str.c_str(), nullptr);
}
template <> inline
double ConvertString::convert_numeric<double>(const std::string& str) {
  return strtod(str.c_str(), nullptr);
}
template <> inline
long long ConvertString::convert_numeric<long long>(const std::string& str) {
  return strtoll(str.c_str(), nullptr, 10);
}
template <> inline
unsigned long long ConvertString::convert_numeric<unsigned long long>(const std::string& str) {
  return strtoull(str.c_str(), nullptr, 10);
}
template <> inline
bool ConvertString::convert_numeric<bool>(const std::string& str) {
  if (strcmp(str.c_str(), "true") == 0 || strcmp(str.c_str(), "True") == 0 ||
      strcmp(str.c_str(), "TRUE") == 0)
  {
    return true;
  }
  ...
  else if (!(strcmp(str.c_str(), "false") == 0 || strcmp(str.c_str(), "False") == 0 ||
               strcmp(str.c_str(), "FALSE") == 0))
  {
    CMLMessage::error(
      __FILE__, __LINE__, "Unrecognized Boolean String\n",
      "The string ", str.c_str(), " did not match \"true\" or \"false\" or equivalent.\n"
