# Information

<p align="center">
    <table>
        <tr>
            <th colspan=5>Model Information</th>
        </tr>
        <tr>
            <td>Rating</td>
            <td colspan=4 align=center>~maturity-3</td>
        </tr>
        <tr>
            <td>POC</td>
            <td>Gary Turner @gturner </td>
            <td>gary.w.turner@nasa.gov</td>
            <td>281.244.0668</td>
            <td>~ANTARES</td>
        </tr>
    </table>
</p>

## Synopsis

An events handling framework that provides more complex triggering options,
including independent arming, disarming, and actionable triggers.
slow down the simulation.

## Documentation

This model is partially documented in the docs directory.

## Verification

This model contains independent verification test cases in the verif directory.
