...
class GustModel
{
 protected:
  ...
  bool   period_message_sent{false}; /* (--)
 ...
 public:
  ...
  double maximum_magnitude{0.0}; /* (m/s) Maximum gust speed */
  double magnitude{0.0};         /* (m/s) Actual gust speed */
  double gust_vector[3]{};    /* (m/s)
    Wind gust vector in frame spec. by gust_frame */
  double period{0.0};            /* (s)   The period of the gust waveform (1/Hz) */
  bool   start{false};             /* (--)
