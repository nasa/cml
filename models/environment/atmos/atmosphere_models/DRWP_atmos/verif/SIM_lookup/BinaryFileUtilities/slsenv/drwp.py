...
def write_binary(w,outfile="DRWP.bin",use_w=False):
   ...
   with open(outfile,'wb') as bf:
      ...
      bf.write(struct.pack(f'{nBPoints}{precision}',*(w['alt'])))
      # 4*nProfiles  int      Wind profile numbers 2, 4, 6, 8,...
      print(wind_set)
      bf.write(struct.pack(f'{len(wind_set)}I',*wind_set))
      ...
      for WINDNUM in wind_set:
         ...
         bf.write(struct.pack(f'{nBPoints}{precision}',*(w[WINDNUM]['u'])))
         # 4*nAlts      float    V wind velocity for wind profile N (lu_v_array)
         bf.write(struct.pack(f'{nBPoints}{precision}',*(w[WINDNUM]['v'])))

         if use_w:
            # 4*nAlts      float    W wind velocity for wind profile N (lu_w_array)
            bf.write(struct.pack(f'{nBPoints}{precision}',*(w[WINDNUM]['w'])))

         # 4*nAlts      float    Temperature for wind profile N (lu_temp_array)
         bf.write(struct.pack(f'{nBPoints}{precision}',*(w[WINDNUM]['temp'])))
         # 4*nAlts      float    Density for wind profile N (lu_rho_array)
         bf.write(struct.pack(f'{nBPoints}{precision}',*(w[WINDNUM]['rho'])))
         # 4*nAlts      float    Pressure for wind profile N (lu_press_array)
         bf.write(struct.pack(f'{nBPoints}{precision}',*(w[WINDNUM]['press'])))